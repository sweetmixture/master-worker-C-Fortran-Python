...
ptasks = int(mp.cpu_count() / 2) # assuming intel hyperthreading provides 2 logic per 1 phys


# Worker Pool Configuration
workgroup     = [ mp.Pool(1) for i in range(ptasks) ]
worker_status = [ 'idle' for i in range(ptasks) ]
job_returns   = [ None for i in range(ptasks) ]


print(f'System CPU(physical) count: {ptasks}')
