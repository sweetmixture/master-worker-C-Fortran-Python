...
void master( MPI_Comm* BaseComm, MPI_Comm* WorkgroupComm, const WorkgroupConfig* workgroup_config, const int n_workgroup )
{
	...
	//printf("send message to: %d tag: %d\n",workgroup_config[1].base_rank,TF_WORKTAG);
	//printf("master brank: %d to workgroup %d - brank %d \n",workgroup_config[n_workgroup-1].base_rank, 1, workgroup_config[1].base_rank);
...
int main(int argc, char* argv[])
{
	...
	if ( workgroup_tag == 0 && worker_rank == 0 ){
		...
		printf("!!! workgroup_tag %d, worker_rank %d, worker_brank %d\n", workgroup_tag, worker_rank, workgroup_config[workgroup_tag].base_rank);
		MPI_Recv(&ft,sizeof(function_task),MPI_CHAR,bsize-1,TF_WORKTAG,BaseComm,&status);
		printf("from head workgroup[0] - task_status: %d\n",ft.task_status);
	}
	...
	if( workgroup_tag == 0 ){
		...
		for(int i=0;i<workgroup_size;i++){
			if( worker_rank == i ){
				printf("wg 0 bcast message %d from internal rank %d / %d\n",ft.task_status,worker_rank,workgroup_size);
			}
		}
	...
	if ( workgroup_tag == 1 && worker_rank == 0 ){
		...
		MPI_Recv(&ft,sizeof(function_task),MPI_CHAR,bsize-1,TF_WORKTAG,BaseComm,&status);
		ft.task_status += 100;
		printf("from head workgroup[1] - task_status: %d\n",ft.task_status);
	}
	...
	if ( workgroup_tag == 1 ){
		...
		for(int i=0;i<workgroup_size;i++){
			if( worker_rank == i ){
				printf("wg 1 bcast message %d from internal rank %d %d\n",ft.task_status,worker_rank,workgroup_size);
			}
		}
